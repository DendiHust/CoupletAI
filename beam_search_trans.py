...
def beam_decode(src_input, model: Transformer, beam_with=3, topk=1):
    ...
    for batch_index in range(batch_size):
        # 当前句子的编码器输出
        encoder_output_current = encoder_outputs[batch_index,:, :].unsqueeze(0)
        ...
        while True:
            ...
            if n.word_index.item() == model.eos_idx and n.previous_node != None:
                endnodes.append((score, n))
                if len(endnodes) > number_required:
                    break
                else:
            ...
            # 解码
            output = model.decoder(trg, encoder_output_current, trg_mask, src_mask)
            ...
            for new_k in range(beam_with):
                ...
                node = BeamSearchNode(trg_tmp, n, decoded_t, log_p+n.log_prob, n.length + 1)
                score = - node.eval()
                next_nodes.append((score, node))
